...
export function UsersPage() {
      ...
      if (modal === 'create') {
        await api('/api/users', {
          ...
          body: {
            ...
            registrationFee:
              form.role === 'student' ? (form.registrationFee === '' ? 0 : Number(form.registrationFee)) : undefined,
            salary: form.role === 'teacher' ? (form.salary === '' ? 0 : Number(form.salary)) : undefined,
          },
        });
        ...
        await api(`/api/users/${modal.id}`, {
          ...
          body: {
            ...
            ...(form.role === 'student'
              ...
              : {}),
            ...(form.role === 'teacher' ? { salary: form.salary === '' ? 0 : Number(form.salary) } : {}),
          },
        });
      }
      setModal(null);
  ...
  return (
    <>
      <PageHeader
        title="Users"
        subtitle="Directory lists teachers and students only (not the principal). You cannot add another admin. Teachers need a salary; students need class and registration fee."
      />
      <Alert message={error} />
              ...
              {data.users?.length ? (
                data.users.map((u) => (
                  <tr key={u._id}>
                    ...
                    <td>{u.role === 'teacher' && u.salary != null ? Number(u.salary).toFixed(0) : '—'}</td>
                    <td>{classLabel(u.assignedClass)}</td>
                    <td>{u.role === 'student' && u.registrationFee != null ? Number(u.registrationFee).toFixed(0) : '—'}</td>
                    <td>
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                setForm((f) => ({
                  ...f,
                  role,
                  registrationFee: role === 'student' ? f.registrationFee : '',
                  salary: role === 'teacher' ? f.salary : '',
                  assignedClass: role === 'student' ? f.assignedClass : '',
                }));
              }}
            >
